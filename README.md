...
**当前版本：** 1.8 &nbsp;|&nbsp; **许可证：** GPL v2+ &nbsp;|&nbsp; **原作者：** [友人a丶](https://nicen.cn/) &nbsp;|&nbsp; **当前维护者：** Haibin
...
| `[fragments]` | 生活碎片信息流；默认每次加载 5 条，最多在页面中保留 20 条 |
...
| 模板 | 说明 |
|------|------|
| 生活碎片 | 类似朋友圈的短内容信息流，支持图片缩略图、灯箱、内容展开与评论展示 |
...
| 留言板 | 访客留言页面 |
| 友情链接 | 友链展示页面 |
| 阅读统计 | 仅登录用户可见的访问概览、趋势和热门文章面板 |
| 阅读趋势 | 仅登录用户可见的热门文章多折线趋势图 |
...
- SMTP 邮件通知：新评论通知站长、回复通知用户，发布/更新文章时通知订阅邮箱
- 邮箱订阅审核：读者在独立订阅页提交申请后，站长在后台批准后才会收到文章通知
- 文章通知仅发送公开正文文章，生活碎片、moment、私密或密码保护内容不会触发邮件
- 评论反垃圾：蜜罐字段、时间检测、频率限制、内容过滤、自定义黑名单和恶意域名触发的永久 IP 封禁
- 中文文章链接优化：服务器启用 PHP intl 扩展时，新文章默认链接可从中文标题转为拼音 slug
- Markdown 公式转换：在“工具 → 公式转换”中将常见 Markdown 数学语法转换为 WP Githuber MD 的 KaTeX 格式
...
主题提供生活碎片、文章聚合、留言板、阅读统计等页面模板。在后台添加页面时选择所需模板，发布后即可将页面加入导航菜单。
...
2. 评论回复通知被回复评论的用户
3. 评论审核通知通知发布评论的用户
4. 文章通知模式支持下拉选择：关闭、仅发布新文章、发布新文章和更新文章

文章通知使用主题选项里的 SMTP 配置发送。若使用学校邮箱作为发件邮箱，请在 SMTP 邮件账号中填写实际发件邮箱，并配置同一邮箱对应的 SMTP 服务器、端口、协议和授权密码。收件人支持一行一个邮箱，也支持用逗号或分号分隔。默认关闭文章通知；如果希望更新旧文章也提醒读者，可以在下拉框中选择“发布新文章和更新文章”。

开启“显示读者邮箱订阅入口”后，主题会提供 `/subscribe/` 独立订阅页，文章底部按钮会指向该页面。读者提交后不会立刻收到邮件，而是进入后台【邮件订阅】待审核列表；批准后才会并入文章通知收件人。订阅申请带有 nonce、蜜罐、时间签名、IP/邮箱频率限制和人工审批，后台可对申请执行批准、拒绝和删除操作。

邮件通知只会发送公开的正文文章：`post` 类型、`publish` 状态、无密码保护，且不属于 `fragments` / `life-fragments` / `moments` / `moment` 分类。生活碎片、moment、私密内容和密码保护内容不会触发文章邮件。

中文文章标题的默认链接可自动转为拼音 slug，例如中文标题在服务器支持 PHP `intl` 扩展时会生成更适合分享的英文路径。若服务器未启用 `intl`，主题会保留 WordPress 默认 slug，不会影响发文。

#### 7. Markdown 公式转换

主题为 WP Githuber MD 提供了一个粘贴式公式转换工具，不会自动修改或保存文章。

1. 确认 WP Githuber MD 已启用 KaTeX
2. 打开 WordPress 后台的【工具 → 公式转换】
3. 将 Markdown 原文粘贴到左侧，点击【转换】
4. 复制右侧结果并替换文章原文

转换器支持 `$$ ... $$`、独立的 `[ ... ]` / `\[ ... \]`、标题包裹的 `# [ ... ]`、`\( ... \)` 行内公式、Markdown 表格中的括号公式，以及 `(1\ 2)(3\ 4)` 形式的置换循环。已有代码块会保持原样。
...
1. 新增 **生活碎片** 页面模板和 `[fragments]` 短代码：提供短内容、图片缩略图与灯箱、内容展开和条目下方评论
2. 生活碎片采用增量分页，每次加载 5 条，并通过 20 条内容窗口控制长时间滚动时的 DOM 与内存占用
3. 首页支持返回时恢复已加载分页和滚动位置
4. 新增 **Markdown 公式转换** 工具，支持块公式、行内公式、表格公式、异常 Markdown 和置换循环表达式，并修复 KaTeX 公式被代码块配色染成浅蓝色的问题
5. 评论反垃圾新增可扩展黑名单，并对发送 `www.binance.com` 的未登录访客永久封禁 IP、拒绝评论和后续访问
6. 新增 **阅读统计**、**阅读趋势** 页面和文章阅读趋势迷你图
7. 新增 Service Worker 离线阅读与 RSS 封面图、摘要优化
8. 新增 **3D 模型查看器**、**3D 交互图表**和**图片对比滑块**短代码
9. 修复暗色模式页面导航时背景闪白问题
10. 新增 GitHub Actions CI，在 push / PR 时自动运行构建和 PHP 语法检查
